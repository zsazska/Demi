...
scoreboard players add $temp demi_previousvalue 16777216
execute store result entity @e[tag=modify,limit=1] background int 1 run scoreboard players get $temp demi_previousvalue
scoreboard players add @e[tag=modify,limit=1] background_alpha 1
execute if score @e[tag=modify,limit=1] background_alpha matches 256.. run scoreboard players set @e[tag=modify,limit=1] background_alpha 0
