...
execute as @e[tag=modify,limit=1] store result score @s demi_previousvalue run data get entity @s Rotation[0] 1
execute as @e[tag=modify,limit=1] run scoreboard players remove @s demi_previousvalue 10
execute as @e[tag=modify,limit=1] store result entity @s Rotation[0] float 1 run scoreboard players get @s demi_previousvalue
